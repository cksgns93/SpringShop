	...
	<div align="center" id="bbs" class="col-md-10 offset-md-1">
		...
		<table class="table table-condensed table-striped">
			...
			<c:forEach var="board" items="${boardList}">
				<tr>
					<td>${board.idx}</td>
					<td align="left"><a href="view?idx=${board.idx}">
							${board.subject} </a></td>
					<td>${board.name}</td>
					...
					<ul class="pagination jutify-content-center">
						<c:if test="${paging.prevBlock>0}">
							<li class="page-item"><a class="page-link"
								href="list?cpage=${paging.prevBlock}&pageSize=${paging.pageSize}">Prev</a>
							</li>
						</c:if>
						<c:forEach var="i" begin="${paging.prevBlock+1}"
							end="${paging.nextBlock-1}">
							<c:if test="${i<=paging.pageCount }">
								<li
									class="page-item <c:if test='${i eq paging.cpage }'>active</c:if>">
									<a class="page-link"
									href="list?cpage=${i}&pageSize=${paging.pageSize}">${i}</a>
								</li>
							</c:if>
						</c:forEach>
						<c:if test="${paging.nextBlock<=paging.pageCount}">
							<li class="page-item"><a class="page-link"
								href="list?cpage=${paging.nextBlock}&pageSize=${paging.pageSize}">Next</a>
							</li>
						</c:if>
					</ul>
				</td>
